...
Ext.ux.TouchCalendarView = Ext.extend(Ext.DataView, {
	...
	/**
	 * Adjust the date for the exchange, daylight saving time.
   * Inpisrate in jQuery UI Datepicker solution.
	 * @method
	 * @private
	 * @param {Date} date
	 * @return {Date} date
	 */
	daylightSavingTimeAdjust: function(date) {
    if (!date) return null;
...
Ext.ux.TouchCalendarView.MONTH = {
	...
	getNextIterationDate: function(d, index){
		return this.daylightSavingTimeAdjust(new Date(d.getFullYear(), d.getMonth(), d.getDate() + (index===0?0:1)));
	},
	...
	getStartDate: function(date){
		return Ext.createDelegate(Ext.ux.TouchCalendarView.WEEK.getStartDate, this)(this.daylightSavingTimeAdjust(new Date(date.getFullYear(), date.getMonth(), 1)));
	},
	...
	getDeltaDate: function(date, delta){
		var newMonth = date.getMonth() + delta,
			newDate = this.daylightSavingTimeAdjust(new Date(date.getFullYear(), newMonth, 1));
...
Ext.ux.TouchCalendarView.WEEK = {
	...
	getStartDate: function(date){
		...
		return this.daylightSavingTimeAdjust(new Date(date.getFullYear(), date.getMonth(), date.getDate()-0-dayOffset));
	},
	 ...
	 */
	getDeltaDate: function(date, delta){
		return this.daylightSavingTimeAdjust(new Date(date.getFullYear(), date.getMonth(), date.getDate() + (delta * 7)));
	},
...
Ext.ux.TouchCalendarView.DAY = {
	...
	getNextIterationDate: function(currentIterationDate, index){
		...
		return this.daylightSavingTimeAdjust(new Date(d));
	},
	 ...
	 */
	getDeltaDate: function(date, delta){
		return this.daylightSavingTimeAdjust(new Date(date.getFullYear(), date.getMonth(), date.getDate() + delta));
	},
